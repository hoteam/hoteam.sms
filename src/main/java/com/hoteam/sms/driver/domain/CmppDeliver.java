    ...
    public CmppDeliver(byte[] data) {
        ...
        try {
            if (null != data && data.length > 0) {
                ...
                dis.read(msgid_b);
                this.Msg_Id = msgid_b;
                this.Dest_Id = MessageTool.readString(dis, 21, null);// 21bit
                this.Service_Id = MessageTool.readString(dis, 10, null);
                this.TP_pid = dis.readByte();
                this.TP_udhi = dis.readByte();
                this.Msg_Fmt = dis.readByte();
                // 源终端MSISDN号码（状态报告时填为CMPP_SUBMIT消息的目的终端号码）
                this.Src_terminal_Id = MessageTool.readString(dis, 21, this.Msg_Fmt == 8 ? "UTF-16BE" : "gb2312");
                ...
                if (this.Registered_Delivery == 1) {
                    ...
                    this.Reserved = dis.readUTF();// 保留项 暂无用
                }
                this.result = 0;
            } else {
                this.result = 1;
